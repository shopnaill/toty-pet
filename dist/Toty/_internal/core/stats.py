...
class PersistentStats:
    ...
    def _load(self):
        if os.path.exists(STATS_PATH):
            try:
                # Reject files over 10 MB — anything that large is corrupt
                if os.path.getsize(STATS_PATH) > 10 * 1024 * 1024:
                    os.remove(STATS_PATH)
                    return
                with open(STATS_PATH, "r", encoding="utf-8") as f:
                    self.data.update(json.load(f))
            except (json.JSONDecodeError, IOError, MemoryError,
                    UnicodeDecodeError, ValueError):
                # Corrupted file — remove it so a fresh one is created
                try:
                    os.remove(STATS_PATH)
                except OSError:
                    pass
